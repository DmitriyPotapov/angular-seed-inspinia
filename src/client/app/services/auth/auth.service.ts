...
export class AuthService {
    ...
    login(username: string, password: string): Observable<any> {
        ...
        client_id=${encodeURI(this.credentials.client_id)}
        &client_secret=${encodeURI(this.credentials.client_secret)}
        &username=${encodeURI(this.credentials.username)}
        &password=${encodeURI(this.credentials.password)}
        &grant_type=${encodeURI(this.credentials.grant_type)}
        &response_type=${encodeURI(this.credentials.response_type)}`;
    ...
    private onLoginRequestSuccess(res: Response) {
        localStorage.setItem('id_token', JSON.stringify(res.json()));
        return true;
    }
